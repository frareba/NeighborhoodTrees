...
public class KNeighborhoodTree<V, E extends Edge> extends GraphEncoder<V, E> {
	...
	private int k;
	  ...
	  public KNeighborhoodTree(AbstractGraph<V, E> graph, int k)
	  {
	    super(graph);
	    this.k = k;
	  ...
	  public KNeighborhoodTree(AbstractGraph<V, E> graph, boolean refine, int k)
	  {
	    this(graph, k);
	   ...
	   */
	  public KNeighborhoodTree(AbstractGraph<V, E> graph, int refinementsteps, int k)
	  {
	    this(graph, k);
	      ...
	      HashMap<V, Integer> depthMap)
	  {
	    ...
	    for (VertexWrapper<V> leaf : leaves)
	    {
	      ...
	      for (E e : graph.edgesOf(v))
	      {
	        ...
	        else if (nextLayer <= distanceToRoot + k)
	        {
